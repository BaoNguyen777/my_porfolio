...
export default function Hero() {
  return (
    <section id="home" className="mx-auto mt-14 grid w-[92%] grid-cols-1 gap-10 lg:grid-cols-2">
      <Left />
      <Right />
    </section>
  );
    ...
    <div className="grid gap-6">
      <div className="flex items-center gap-4 rounded-xl border border-emerald-500/20 bg-black/40 p-4 glow">
        ...
        <div className="space-y-1 text-sm">
          ...
          <p>🌐 baonguyn.vercel.com</p>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-4">
        <Stat v="3+" l="Năm kinh nghiệm Web" icon={Briefcase} />
        <Stat v="10+" l="Dự án hoàn thành" icon={FolderKanban} />
        <Stat v="100%" l="Chuẩn UX/UI" icon={Palette} />
        <Stat v="100%" l="Độ tin cậy" icon={ShieldCheck} />
      </div>
    </div>
  );
}
...
function Stat({
  ...
  return (
    <div className="rounded-xl border border-emerald-500/20 bg-black/40 p-4 text-center glow space-y-2">
...
function FloatingName() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      frame += 0.03;
      setOffset(Math.sin(frame) * 12); // biên độ lên xuống
